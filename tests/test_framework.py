...
print(f"  敏感度场形状: {field_grid.shape}")
...
# 测试4: 测试覆盖控制器
print("\n[4/6] 测试覆盖控制器...")
controller = LloydController(domain, gain=1.5, max_velocity=5.0, resolution=RESOLUTION)
print(f"  控制器分辨率: {controller.resolution}")
print(f"  Voronoi网格点数: {len(controller.voronoi.grid_points)}")
print(f"  敏感度场大小: {field_grid.size}")
...
for i, cell in enumerate(cells):
    print(f"    UAV {i}: 位置={positions[i]}, 质心={cell.centroid}, 质量={cell.mass:.2f}")
